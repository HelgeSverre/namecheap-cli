...
import { updateAddress, type AddressInput } from '../../lib/api/address.js';
...
export const updateCommand = new Command('update')
  ...
  .argument('<id>', 'Address ID to update')
  ...
  .action(async (id: string, options: UpdateOptions) => {
    try {
      ...
      const result = await withSpinner('Updating address...', async () => {
        return updateAddress(client, id, input);
      });

      if (options.json) {
        output(result, options);
      } else {
        success(`Address "${result.addressName}" (ID ${result.addressId}) updated`);
      }
    } catch (error) {
      handleError(error);
    }
